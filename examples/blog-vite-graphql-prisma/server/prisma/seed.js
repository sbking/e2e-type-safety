...
async function seed() {
    ...
    const now = new Date();
    const postBodies = await Promise.all(Array.from({ length: 5 }).map(generateMarkdown));
    // Create between 10-20 posts
    const posts = Prisma.validator()(Array.from({ length: faker.datatype.number({ min: 10, max: 20 }) }, () => {
        ...
        return {
            ...
            imageUrl: faker.image.unsplash.image(640, 480, faker.helpers.unique(faker.hacker.noun)),
            createdAt,
            updatedAt: faker.date.between(createdAt, now),
        };
    }));
    // Create between 0-20 comments per post
    const comments = Prisma.validator()(posts.flatMap((post) => Array.from({ length: faker.datatype.number({ min: 0, max: 20 }) }, () => ({
        postSlug: post.slug,
